module.exports = {
    ...
    callback: async (sock, m, { setReply }) => {
        ...
        if (idGroup !== m.chat) await setReply("Success leave group")
        } else if (m.text.includes("@g.us") && !isNaN(m.text.split("@")[0])) {
        await sock.groupLeave(m.text)
        if (m.text !== m.chat) await setReply("Success leave group")
        } else if (m.isGroup) {
        ...
        if (m.text !== "" && data[Number(m.text) +1] !== m.chat) await setReply("Success leave group")
        } else if (Object.keys(metandaId).includes(m.text)) {
        await sock.groupLeave(metandaId[m.text].id)
        if (metandaId[m.text].id !== m.chat) await setReply("Success leave group")
        } else {
        setReply("Id?") 
        }
    }
